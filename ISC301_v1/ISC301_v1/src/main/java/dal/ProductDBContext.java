...
public class ProductDBContext extends DBContext {
    ...
    public List<Product> getProductsWithPaging(int page, int PAGE_SIZE) {
        List<Product> list = new ArrayList<>();
        try {
    ...
    public List<Product> getProductsWithPagingAndSort(int page, int PAGE_SIZE, String type) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "SELECT * FROM Product ORDER BY price " + type + " LIMIT ?, ?";
    ...
    public List<Product> search(String keyword) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "SELECT * FROM Product WHERE name LIKE ?"; // Use % as the wildcard for MySQL

            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setString(1, "%" + keyword + "%"); // Add % before and after the keyword
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Product product = new Product();
                product.setId(rs.getInt(1));
                ...
                product.setImageUrl(rs.getString(3));
                product.setPrice(rs.getDouble(4));
                product.setTiltle(rs.getString(5)); // Corrected the method name
                product.setDescription(rs.getString(6));
                product.setCategoryId(rs.getInt(7));
                product.setSell_ID(rs.getInt(8));
                list.add(product);
    ...
    public void updateProduct(Product p) {
        try {
            String sql = "UPDATE product "
                    + "SET name = ?, image = ?, price = ?, title = ?, description = ?, cateID = ?, sell_ID = ? "
                    + "WHERE id = ?";
    ...
    public List<Product> sort(String type) {
        List<Product> list = new ArrayList<>();
